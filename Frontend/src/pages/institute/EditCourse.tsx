...
import { useCoursedetailQuery, useModuleDeleteMutation, useUpdateCourseMutation } from '../../store/slices/institutionSlice';
...
const EditCourseDetail = () => {
  ...
  const handleSubmit = async (values: CourseFormValues): Promise<void> => {
    try {
      ...
      // navigate('/institute/upload-videos', { state: { draftId: response.data._id } });
    } catch (error: any) {
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold">Course Modules Updation</h2>
                <button
                  onClick={() => navigate('/institute/upload-videos', { state: { draftId: course?._id } })}
