...
interface ReviewSectionProps {
  Review: any;
  loggedInUserId: string; // Add the logged-in user's ID
  isAddingReview: boolean;
  setIsAddingReview: (isAdding: boolean) => void;
  currentValue: number | undefined;
  setCurrentValue: (value: number | undefined) => void;
...
const ReviewSection: React.FC<ReviewSectionProps> = ({
  ...
  }, [Review, loggedInUserId, setCurrentValue, setReviewText]);
  ...
  return (
    <div className="mt-8 bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        >
          <Plus className="w-4 h-4 mr-2" />
          {currentValue ? "Edit Review" : "Add Review"}
        </button>
      </div>
        ...
        <div className="mb-6 bg-gray-50 p-6 rounded-lg">
          ...
          <div className="flex justify-between items-center mt-4">
            <button
              ...
              className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
            >
              {currentValue ? "Update Review" : "Submit Review"}
            </button>
            <button
              onClick={() => setIsAddingReview(false)}
              className="text-gray-600 hover:text-gray-800 transition-colors"
