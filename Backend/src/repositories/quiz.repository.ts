...
export class QuizRepository extends BaseRepository<QuizDocument> {
    ...
    async findOne(institutionId:string,searchQuery:SearchQuiz,skip:number,limit:number,  sortOptions: any = { createdAt: -1 }): Promise<{quiz:QuizDocument[]; total: number; department:string[]}> {
        try {            
            const quiz = await this.model.find({institutionId:institutionId,...searchQuery})
            .sort(sortOptions)
            .skip(skip)
            .limit(limit)
            .populate({
